...
class _RootScreenState extends State<RootScreen> {
  ...
  final _baseUrlController = TextEditingController(
    text: 'http://192.168.1.182:5006',
  );
  final _serverHint =
      'LAN: http://192.168.1.182:5006 | Android emulator: http://10.0.2.2:5006';
  ...
  // Budgets are shown on a separate screen after login.
  ...
  Future<void> _handleIncomingUri(Uri uri) async {
    ...
    if (api == null) {
      setState(
        () => _error = 'Received OpenID token but API not connected yet',
      );
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      // Immediately transition into budget selection screen.
      ...
      setState(
        () => _error = 'OpenID token received, but loading budgets failed: $e',
      );
    }
  }
  ...
  Future<void> _loginAndSelectBudget() async {
    final api = _api;
    if (api == null) return;
    final pwd = _passwordController.text;
    if (pwd.isEmpty) {
            ...
            child: Column(
              ...
              children: [
                ...
                if (api != null) ...[
                  ...
                  FilledButton.icon(
                    onPressed: _loading ? null : _loginAndSelectBudget,
                ...
                const SizedBox(height: 12),
                const Text(
                  'After login, budget selection opens in a separate screen.',
